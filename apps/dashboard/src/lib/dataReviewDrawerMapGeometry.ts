...
/** Loose row shape for drawer map helpers (core-review and legacy IR rows). */
export type DataReviewMapRow = {
    geometry?: DataReviewGeoJson | null;
    geom?: DataReviewGeoJson | null;
...
export function normalizeDataReviewGeoJson(raw: unknown): Geometry | null {
    ...
    if (t === "Feature") {
        return normalizeDataReviewGeoJson(o.geometry);
    }

    if (t === "FeatureCollection" && Array.isArray(o.features)) {
        for (const feature of o.features) {
            const g = normalizeDataReviewGeoJson(feature);
            if (g) {
                return g;
            }
        }
    ...
    if (t === "GeometryCollection" && Array.isArray(o.geometries)) {
        for (const g of o.geometries) {
            const normalized = normalizeDataReviewGeoJson(g);
            if (normalized) {
                return normalized;
            }
        }
...
function pickReviewOverridesField(row: DataReviewMapRow, key: string): unknown {
    const ov = row.fields;
...
export function buildingDrawerMapInput(row: DataReviewMapRow): {
    geometry: DataReviewGeoJson | null;
    geometryKind: DataReviewGeometryKind;
    fallbackNote: string | null;
} {
    for (const candidate of [row.geometry, row.geom, pickReviewOverridesField(row, "geom")]) {
        const g = normalizeDataReviewGeoJson(candidate);
        if (g?.type === "Polygon" || g?.type === "MultiPolygon") {
            return { geometry: g as unknown as DataReviewGeoJson, geometryKind: "polygon", fallbackNote: null };
        }
        if (g?.type === "Point" || g?.type === "MultiPoint") {
            return { geometry: g as unknown as DataReviewGeoJson, geometryKind: "point", fallbackNote: null };
        }
    }
    const c = row.centroid as DataReviewGeoJson | null | undefined;
...
export function placeDrawerMapInput(row: DataReviewMapRow): {
    geometry: DataReviewGeoJson | null;
    geometryKind: DataReviewGeometryKind;
    fallbackNote: string | null;
} {
    for (const candidate of [row.geometry, row.geom]) {
        const poly = normalizeDataReviewGeoJson(candidate);
        if (poly?.type === "Polygon" || poly?.type === "MultiPolygon") {
            return {
                geometry: poly as unknown as DataReviewGeoJson,
                geometryKind: "polygon",
                fallbackNote: null,
            };
        }
    }
    const g = normalizeDataReviewGeoJson(row.geometry);
    if (g?.type === "Point" || g?.type === "MultiPoint") {
        return { geometry: g as unknown as DataReviewGeoJson, geometryKind: "point", fallbackNote: null };
    }
    const wkt = pickCentroidWktFromNormalized(row.normalized_data);
...
export function roadDrawerMapInput(row: DataReviewMapRow): {
    geometry: DataReviewGeoJson | null;
    geometryKind: DataReviewGeometryKind;
    fallbackNote: string | null;
} {
    ...
    for (const raw of sources) {
        ...
        if (line) {
            return {
                geometry: line as unknown as DataReviewGeoJson,
                geometryKind: "line",
                fallbackNote: null,
            };
        }
...
export function entityDrawerMapInput(
    ...
    for (const raw of sources) {
        const g = normalizeDataReviewGeoJson(raw);
        if (g) {
            return {
                geometry: g as unknown as DataReviewGeoJson,
                geometryKind: "polygon",
                fallbackNote: null,
            };
        }
