...
import { TourismReviewDetailDrawer } from "./TourismReviewDetailDrawer";
import type { TourismReviewStatus } from "./types";
...
export default function TourismReviewsPage() {
            ...
            <tbody className="divide-y divide-gray-100">
              {items.map((item) => (
                <tr key={item.public_id} className="hover:bg-gray-50/80">
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`inline-flex rounded-full px-2 py-0.5 text-xs font-semibold ring-1 ${tourismStatusBadgeClass(item.status as TourismReviewStatus)}`}
                    >
                      {tourismStatusLabel(item.status as TourismReviewStatus)}
                    </span>
                  </td>
