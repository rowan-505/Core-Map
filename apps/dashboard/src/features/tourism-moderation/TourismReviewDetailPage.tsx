...
import { TourismModerationDialog } from "./TourismModerationDialog";
import { ModerationHistoryList } from "./TourismReviewDetailDrawer";
import type { TourismModerationAction } from "./types";
...
export default function TourismReviewDetailPage({
  publicId,
}: {
  readonly publicId: string;
}) {
  const queryClient = useQueryClient();
  const [pendingAction, setPendingAction] = useState<TourismModerationAction | null>(null);
  ...
  const moderateMutation = useMutation({
    mutationFn: (input: { action: TourismModerationAction; note?: string }) =>
