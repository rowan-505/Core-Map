...
import { listAdminTourismGuides } from "./api";
...
export default function TourismGuidesPage() {
  const router = useRouter();
  const pathname = usePathname();
  const searchParams = useSearchParams();

  const adminAreaId = searchParams.get("admin_area_id")?.trim() || "";
  const guideType = searchParams.get("guide_type")?.trim() || "";
  ...
  const filters = useMemo(
    ...
    [adminAreaId, guideType, isActive, isVerified, q, offset],
  );

  const listQuery = useQuery({
    queryKey: ["tourism-visitor", "guides", filters],
    queryFn: ({ signal }) => listAdminTourismGuides(filters, { signal }),
    placeholderData: keepPreviousData,
  });
  const areasQuery = useQuery({
    queryKey: ["admin-area-options", "tourism-guides", areaQuery],
    queryFn: () =>
  ...
  return (
    <div className="space-y-4 p-4 md:p-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-xl font-semibold text-gray-900">Local guides</h1>
          <p className="mt-1 text-sm text-gray-600">Visitor tips and local knowledge by area.</p>
        </div>
        <Link href={tourismPath("guides/new")} className={PRIMARY_BTN}>
          New guide
        </Link>
      </div>
      ...
      <div className="flex flex-wrap items-end gap-2">
        ...
        <label className="text-sm text-gray-700">
          ...
          <select
            value={guideType}
            onChange={(e) => replaceFilters({ guide_type: e.target.value || null })}
      ...
      {listQuery.isError ? (
        <p className="text-sm text-red-700">
          {listQuery.error instanceof Error ? listQuery.error.message : "Failed to load guides"}
        </p>
      ) : null}
            ...
            {items.map((row) => (
              <tr key={row.public_id} className="border-b border-gray-100 hover:bg-gray-50">
                <td className="px-3 py-2">
                  <Link
                    href={tourismPath(`guides/${row.public_id}`)}
                    className="font-medium text-gray-900 hover:underline"
                  >
                    {row.title}
                  </Link>
                </td>
                <td className="px-3 py-2 text-gray-700">{row.guide_type}</td>
                <td className="px-3 py-2 text-gray-700">{row.admin_area_name}</td>
                <td className="px-3 py-2 text-gray-700">{row.place_name ?? "—"}</td>
