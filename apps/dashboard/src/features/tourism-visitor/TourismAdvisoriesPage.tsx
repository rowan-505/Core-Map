...
import { listAdminTourismAdvisories } from "./api";
...
export default function TourismAdvisoriesPage() {
  const router = useRouter();
  const pathname = usePathname();
  const searchParams = useSearchParams();

  const adminAreaId = searchParams.get("admin_area_id")?.trim() || "";
  const advisoryType = searchParams.get("advisory_type")?.trim() || "";
  ...
  const filters = useMemo(
    () => ({
      ...
      advisory_type: advisoryType || undefined,
    ...
    [adminAreaId, advisoryType, severity, isActive, isVerified, q, offset],
  );

  const listQuery = useQuery({
    queryKey: ["tourism-visitor", "advisories", filters],
    queryFn: ({ signal }) => listAdminTourismAdvisories(filters, { signal }),
    placeholderData: keepPreviousData,
  });
  const areasQuery = useQuery({
    queryKey: ["admin-area-options", "tourism-advisories", areaQuery],
    queryFn: () =>
  ...
  return (
    <div className="space-y-4 p-4 md:p-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-xl font-semibold text-gray-900">Advisories</h1>
          <p className="mt-1 text-sm text-gray-600">Visitor advisories by area and severity.</p>
        </div>
        <Link href={tourismPath("advisories/new")} className={PRIMARY_BTN}>
          New advisory
        </Link>
      </div>
      ...
      <div className="flex flex-wrap items-end gap-2">
        ...
        <label className="text-sm text-gray-700">
          ...
          <select
            value={advisoryType}
            onChange={(e) => replaceFilters({ advisory_type: e.target.value || null })}
        ...
        <label className="text-sm text-gray-700">
          ...
          <select
            value={severity}
            onChange={(e) => replaceFilters({ severity: e.target.value || null })}
            className={`${SELECT_CLASS} ml-2`}
          >
            ...
            {items.map((row) => (
              <tr key={row.public_id} className="border-b border-gray-100 hover:bg-gray-50">
                <td className="px-3 py-2">
                  <Link
                    href={tourismPath(`advisories/${row.public_id}`)}
                    className="font-medium text-gray-900 hover:underline"
                  >
                    {row.title}
                  </Link>
                </td>
                <td className="px-3 py-2 text-gray-700">{row.advisory_type}</td>
                <td className="px-3 py-2 text-gray-700">{row.severity}</td>
