...
import { DeleteLandError, parseDeleteLandFeatureKey, runDeleteLand } from "./delete-land.lib.ts";
...
async function writeSuppression(featureKey: string): Promise<{ created: boolean; core_removed: boolean }> {
  const { status, body } = await adminPost("/core-review/land-areas/delete-from-source", {
...
function sync(): Promise<{ ok: boolean; detail: string }> {
  return new Promise((resolvePromise) => {
    const child = spawn("npm", ["run", "tiles:sync", "--", "land_areas_suppressed", "land_areas_core"], {
      cwd: repoRoot,
      env: process.env,
...
async function verifyAbsent(
  ...
  const resolved = await client.query<{ n: string }>(
    `SELECT COUNT(*)::text AS n FROM tile_source.land_areas_v WHERE feature_key = $1`,
    [featureKey]
  );
  const suppressed = await client.query<{ n: string }>(
    `SELECT COUNT(*)::text AS n FROM tile_source.land_areas_suppressed WHERE feature_key = $1`,
    [featureKey]
  );
  return {
...
async function main(): Promise<number> {
  ...
  parseDeleteLandFeatureKey(rawKey);

  const localUrl = process.env.LOCAL_TILE_DATABASE_URL || process.env.COREMAP_TILES_DATABASE_URL;
  if (!localUrl) {
    throw new DeleteLandError("config", "LOCAL_TILE_DATABASE_URL is required.", 1);
  }
