...
async function lookupLocal(client: pg.Client, identity: LandOsmIdentity): Promise<LocalLandLookup> {
    ...
    `SELECT TRUE AS suppressed
     WHERE EXISTS (
       ...
       WHERE feature_key = $1
        ...
        OR EXISTS (
       SELECT 1
       FROM tile_source.land_areas_suppressed
       WHERE feature_key = $1
     )`,
    [identity.featureKey]
  );
  ...
  const archive = await client.query<{
    ...
    name_mm: string | null;
    name_en: string | null;
    geometry: LocalLandPromoteCandidate["geometry"] | null;
  }>(
  ...
  const base = await client.query<{
    ...
    `SELECT
        ...
        ${GEOM_SQL} AS geometry
     FROM tile_source.land_areas_base
  ...
  const toCandidate = (
    ...
    row:
      ...
      | undefined
  ): LocalLandPromoteCandidate | null => {
    if (!row?.geometry) {
      return null;
    }
    return {
...
async function promoteViaApi(payload: {
  ...
  const response = await fetch(`${apiBaseUrl()}/core-review/land-areas/promote-from-source`, {
    method: "POST",
    headers,
    body: JSON.stringify(payload),
  });
  const body = (await response.json().catch(() => null)) as
    | (PromoteLandApiResult & { message?: string })
    | null;
...
function syncLandAreasCore(): Promise<{ ok: boolean; detail: string }> {
  return new Promise((resolvePromise) => {
    const child = spawn("npm", ["run", "tiles:sync", "--", "land_areas_core"], {
      cwd: repoRoot,
      env: process.env,
...
async function main(): Promise<number> {
  ...
  parseLandPromoteFeatureKey(rawKey);

  const localUrl = process.env.LOCAL_TILE_DATABASE_URL || process.env.COREMAP_TILES_DATABASE_URL;
  if (!localUrl) {
    throw new PromoteLandError("config", "LOCAL_TILE_DATABASE_URL is required.", 1);
  }
...
main()
  ...
  .catch((error) => {
    if (error instanceof PromoteLandError) {
