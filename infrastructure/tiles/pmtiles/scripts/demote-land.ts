...
async function preflight(featureKey: string): Promise<DemotePreflightPayload> {
  const { status, body } = await adminPost("/core-review/land-areas/demote-preflight", { feature_key: featureKey });
  if (status === 404) {
  ...
  return {
    ...
    class_code: String(body.class_code ?? ""),
...
async function writeArchive(client: pg.Client, payload: DemotePreflightPayload): Promise<void> {
  ...
  const updated = await client.query(
    `UPDATE tile_source.land_areas_archive AS a
     SET
       core_id = $2::bigint,
  ...
  const result = await client.query<{
    ...
    `SELECT
       ...
       (
         ...
         AND core_snapshot ? 'land_area_class_id'
         AND core_snapshot ? 'detail_level'
       ) AS snapshot_ok
     FROM tile_source.land_areas_archive
     WHERE feature_key = $1
     ORDER BY demoted_at DESC NULLS LAST, id DESC
...
async function removeCore(featureKey: string): Promise<{ core_id: string }> {
  const { status, body } = await adminPost("/core-review/land-areas/demote-from-core", { feature_key: featureKey });
...
async function main(): Promise<number> {
  ...
  parseDemoteLandFeatureKey(rawKey);

  const localUrl = process.env.LOCAL_TILE_DATABASE_URL || process.env.COREMAP_TILES_DATABASE_URL;
  if (!localUrl) {
    throw new DemoteLandError("config", "LOCAL_TILE_DATABASE_URL is required.", 1);
  }
